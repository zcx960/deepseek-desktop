...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | Web 专用；零 workspace 依赖（注册表经结构注入到达）；首个桌面壳监督普通 Web profile 子进程，而不把该载体导入 Electron |
