...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Web only; zero workspace dependencies (the registry arrives by structural injection); the first desktop shell supervises an ordinary Web-profile child rather than importing this carrier into Electron |
