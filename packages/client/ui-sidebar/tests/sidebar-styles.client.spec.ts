...
describe('SidebarRoot.module.css', () => {
  it('keeps desktop drag and safe-area rules behind the shell marker', () => {
    ...
    expect(
      declarations(":global(html[data-dsh-desktop='true']) .brand")?.get('visibility'),
    ).toBe('hidden')
    expect(
      declarations(":global(html[data-dsh-desktop='true']) .brand")?.get('pointer-events'),
    ).toBe('none')
  })
  ...
  it('restores Web rail geometry and removes drag space when embedded in the native shell', () => {
    const root = declarations(":global(html[data-dsh-desktop-embedded='true']) .root")
    const collapsed = declarations(":global(html[data-dsh-desktop-embedded='true']) .root.collapsed")
    expect(root?.get('--dsh-sidebar-collapsed-width')).toBe('56px')
    expect(root?.get('--dsh-sidebar-rail-inline-padding')).toBe('10px')
    expect(root?.get('padding-top')).toBe('6px')
    expect(collapsed?.get('padding-top')).toBe('18px')
    expect(
      declarations(":global(html[data-dsh-desktop-embedded='true']) .titlebarDragRegion")
        ?.get('display'),
    ).toBe('none')
  })

  it('keeps embedded macOS sidebar controls below the native mode switch', () => {
    const marker = ":global(html[data-dsh-desktop-embedded='true'][data-dsh-desktop-platform='darwin'])"
    expect(declarations(`${marker} .root`)?.get('padding-top')).toBe('32px')
    expect(declarations(`${marker} .root.collapsed`)?.get('padding-top')).toBe('48px')
  })
