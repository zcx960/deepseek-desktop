...
在 macOS 上，Electron document 会显示侧栏顶部拖拽条；拖拽条与侧栏内容在 32px 处相接，使 logo 行直接从交通灯下方开始，其内部 60px 几何保持不变。嵌入式双模式 Harness 使用完整窗口边界，因此仍保留 macOS 展开态 32px 与收起态 48px 顶距，同时继续使用 56px 紧凑轨道并禁用侧栏拖拽条。Windows 使用普通的展开态 6px 与轨道态 18px 顶部间距，不显示侧栏拖拽条，因为窗口拖动由会话窗口按钮行负责。存在平台拖拽条时，侧栏控件保持 `no-drag`。Web 不预留原生标题栏空间。在 macOS 与 Windows 上，侧栏根节点保持透明以显示布局持有的原生材质；Linux 仍使用普通侧栏填充色。
