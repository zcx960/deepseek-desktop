...
带有封闭 `dshDesktopTheme` 桥的嵌入式 Desktop renderer 会把初始 `ThemeSnapshot` 及每次 `theme/change` 发布为偏好与解析后的配色。从 Electron 收到的偏好写入只有在属于三个内置值之一时才会传给 `ThemeRuntime.setTheme()`；普通 Web renderer 和无浏览器文档的客户端组合都不会暴露该桥，并保持上述行为。客户端上下文拥有并释放桥订阅。
