...
import type { DesktopMode } from './desktop-mode'
import {
...
export { isDesktopThemePreference } from './desktop-theme'
...
export const DESKTOP_THEME_CHANNELS = {
  ...
  /**
   * Scheme-only report from Harness, whose preference lives in server-side
   * settings rather than in a renderer store main can read.
   */
  scheme: 'dsh-desktop:theme-scheme',
  apply: 'dsh-desktop:theme-apply',
  adapterError: 'dsh-desktop:theme-adapter-error',
} as const

/** Scheme and optional background reported by the Harness renderer. */
export interface DesktopHarnessSchemeReport {
  readonly scheme: DesktopColorScheme
  readonly backgroundColor?: string
}

/**
 * Validate one scheme-only report from the Harness renderer.
 * @param value - Value received from a sandboxed preload.
 * @returns Whether the value carries an accepted scheme.
 */
export function isDesktopHarnessSchemeReport(value: unknown): value is DesktopHarnessSchemeReport {
  if (typeof value !== 'object' || value === null) return false
  const report = value as { scheme?: unknown; backgroundColor?: unknown }
  return isDesktopColorScheme(report.scheme)
    && (report.backgroundColor === undefined || isDesktopThemeBackgroundColor(report.backgroundColor))
}
