...
import { createChatMode, type ChatMode } from './chat-mode'
import { CHAT_PARTITION } from '../shared/chat-navigation'
import { DESKTOP_SHELL_CHANNELS, type DesktopShellCommand } from '../shared/shell-protocol'
import { createDesktopThemeCoordinator, type DesktopThemeCoordinator } from '../shared/desktop-theme-sync'
import type { DesktopMode, DesktopModeSnapshot } from '../shared/desktop-mode'
import {
...
async function syncNativeTheme(window: BrowserWindow): Promise<void> {
  ...
  // Harness paints its own theme and exposes no readable store to a sandboxed
  // preload, so the resolved scheme is reported from here. Pairing it with the
  // stored preference lets the coordinator treat Harness as the authority.
  desktopTheme?.report('harness', {
 ...
 * Chromium.
 */
/** Live Chat mode wiring, or `undefined` before the window exists. */
let chatMode: ChatMode | undefined
/** Shared theme authority across both desktop modes. */
let desktopTheme: DesktopThemeCoordinator | undefined
/** Chrome renderers currently receiving mode snapshots. */
const chromeContents = new Set<WebContents>()
/** Storage key holding the last selected desktop mode. */
const SELECTED_MODE_STORAGE_KEY = 'desktopSelectedMode'

/** Read the persisted mode selection, defaulting to Harness on first launch. */
function savedDesktopMode(): DesktopMode | undefined {
  const value = desktopStorageManager?.getItem(SELECTED_MODE_STORAGE_KEY)
  return value === 'chat' || value === 'harness' ? value : undefined
}

/** Push the current mode snapshot to every trusted chrome renderer. */
function publishModeSnapshot(snapshot: DesktopModeSnapshot): void {
  for (const contents of chromeContents) {
...
/**
 * Route an overlay command that needs the composition root.
 *
 * Reload and clear-data are handled inside the Chat integration itself; only
 * commands that reach Harness or the mobile bridge arrive here.
 */
function runChromeCommand(command: DesktopShellCommand): void {
  switch (command) {
    case 'retry-harness':
      void restartHarness().catch(showUnexpectedError)
      return
    default:
      return
  }
...
/**
 * Attach the Chat overlay and the shared theme coordinator to the main window.
 *
 * Called once during bootstrap, immediately after the window exists. Harness is
 * already the window's own renderer, so only the overlay, the persisted
 * selection, and the theme authority need wiring here.
 *
 * The theme coordinator makes the selected mode authoritative. Harness reports
 * from the server-side `settings.yaml`; Chat reports from its own local storage
 * and accepts writes. Whichever mode is on screen therefore decides the theme,
 * and the other one follows without either side reloading the other.
 */
function installChatMode(): void {
  const window = mainWindow
  if (window === undefined) return

  desktopTheme = createDesktopThemeCoordinator({
    initialMode: savedDesktopMode() ?? 'harness',
    initialSystemScheme: nativeTheme.shouldUseDarkColors ? 'dark' : 'light',
    onChange: (snapshot) => {
      // The native window chrome and the selector both follow the resolved
      // scheme; the renderers paint themselves from their own preference.
      nativeTheme.themeSource = snapshot.preference
      if (!window.isDestroyed()) window.setBackgroundColor(snapshot.scheme === 'dark' ? '#141416' : '#ffffff')
      chatMode?.syncTheme()
    }
  })

  const customSession = session.fromPartition(CHAT_PARTITION)
  chatMode = createChatMode({
    window,
    ipcMain,
    chatThemePreloadPath: join(import.meta.dirname, '../preload/chat-theme.cjs'),
    chromePreloadPath: join(import.meta.dirname, '../preload/mode-chrome.cjs'),
    loadChrome: (contents, query) =>
      loadDesktopResource(contents, desktopResourcePath('mode-chrome.html'), { query }),
    chatSession: customSession,
    openExternal: (url) => shell.openExternal(url),
    readSavedMode: savedDesktopMode,
    writeSavedMode: (mode) => {
      desktopStorageManager?.setItem(SELECTED_MODE_STORAGE_KEY, mode)
      desktopStorageManager?.flushSync()
      return Promise.resolve()
    },
    theme: desktopTheme,
    onCommand: runChromeCommand,
    publishSnapshot: publishModeSnapshot
  })

  // Harness adopts a preference through its own settings surface, so its
  // connection only records what it reports; Chat is the side that can be told
  // to change, which `createChatSurface` already does over its preload channel.
  desktopTheme.connect('harness', () => undefined)

  // Harness reports its scheme from `syncNativeTheme`, which already reads the
  // document it paints; no renderer channel is involved.

  // While the shared preference is `system`, both modes follow the OS as it
  // changes; an explicit light/dark choice keeps the OS out of it.
  nativeTheme.on('updated', () => {
    desktopTheme?.systemChanged(nativeTheme.shouldUseDarkColors ? 'dark' : 'light')
  })

  window.on('closed', () => {
    const live = chatMode
    chatMode = undefined
    desktopTheme = undefined
    chromeContents.clear()
    void live?.dispose()
  })
}
