...
import type { DesktopContentBounds, DesktopMode } from '../shared/desktop-mode'
import type { DesktopChromeSurface } from '../shared/shell-protocol'
...
/**
 * Trailing inset reserved for open mode chrome.
 *
 * This desktop keeps its sidebar rail at the leading edge and its native
 * title-bar drag strip running from 80px to 220px from the right, so the mode
 * switch anchors to the trailing edge rather than after the traffic lights.
 */
const CHROME_TRAILING_INSET = 220
...
export function desktopChromeBounds(input: DesktopChromeBoundsInput): DesktopContentBounds {
  // `input.content` comes from `BrowserWindow.getContentBounds()`, whose x/y are
  // screen coordinates. The selector is a child view, so its rectangle is
  // relative to the window's content area and the screen offset must be
  // dropped — adding it pushes the view off the right edge.
  if (input.surface === 'dialog') {
    return { x: 0, y: 0, width: input.content.width, height: input.content.height }
  }
  const y = CHROME_TOP
  ...
  const availableWidth = Math.max(0, input.content.width - CHROME_TRAILING_INSET)
  const width = Math.min(requestedWidth, availableWidth)
  const availableHeight = Math.max(0, input.content.height - y)
  return {
    x: input.content.width - CHROME_TRAILING_INSET,
    y,
    width,
